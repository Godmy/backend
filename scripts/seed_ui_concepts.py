"""
Скрипт для инициализации UI-переводов
Создает концепты для элементов пользовательского интерфейса и их переводы
"""
...
# Словарь UI-переводов: ключ -> {язык: перевод}
UI_TRANSLATIONS = {
    # Navigation / Header
    "ui/nav/dashboard": {
        "en": "Dashboard",
...
def seed_ui_concepts(db):
    """Создать UI-концепты и их переводы"""
    # Проверяем, есть ли уже UI-концепты
    existing = db.query(ConceptModel).filter(ConceptModel.path.like("ui/%")).first()
    if existing:
        logger.info("UI concepts already exist, skipping...")
        return
    ...
    if missing_langs:
        logger.warning(f"Missing languages: {missing_langs}. Some translations will be skipped.")

    concept_count = 0
    translation_count = 0

    # Создаем концепты и переводы
    for path, translations in UI_TRANSLATIONS.items():
        # Вычисляем глубину
        depth = path.count("/")

        # Найти родительский концепт (если есть)
        parent_id = None
        if "/" in path:
            parent_path = path.rsplit("/", 1)[0]
            parent = db.query(ConceptModel).filter_by(path=parent_path).first()
            if parent:
                parent_id = parent.id
    ...
    db.commit()
    logger.info(f"✓ Added {concept_count} UI concepts with {translation_count} translations")


def main():
    """Запуск seed функции для UI-переводов"""
    logger.info("=" * 60)
    logger.info("Starting UI concepts seeding...")
    logger.info("=" * 60)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("✓ UI concepts seeding completed successfully!")
        logger.info("=" * 60)

    except Exception as e:
        logger.error(f"Error during UI seeding: {e}")
        db.rollback()
        raise
    finally:
        db.close()
