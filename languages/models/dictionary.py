...
class DictionaryModel(BaseModel):
    ...
    concept_id = Column(Integer, ForeignKey("concepts.id", ondelete="CASCADE"), nullable=False, index=True, comment="ID концепции")
    language_id = Column(Integer, ForeignKey("languages.id", ondelete="CASCADE"), nullable=False, index=True, comment="ID языка")
